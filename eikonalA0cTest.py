...
def grid_search(iterations):
    ...
    # Parameters
    tau = round(0.691 * (5/16),3)  # Relaxation time
    ...
    B = [B[arg2]]
    B = [0.36]
    C = np.round(np.linspace(10, 20, gridResol),2)
    C = [C[arg3]]
    C = [10]
    D = np.round(np.linspace(0.1, 0.5, gridResol),2)
    D = [D[arg4]]
    D = [0.2]
        ...
        for b in B:
            for c in C:
                ...
                for d in D:
                    ...
                    total_overlaps, walls_overlaps, ped_left, dens = main_loop(a*(16/5), b, c*(16/5), d, kn, kt, room_width, room_height, door_width, exit_doorx,exit_doory, num_pedestrians, num_iterations, dt, deleted, tau, m, v0, l, positions, wallsy, wallsx, velocities, r, iterations)
