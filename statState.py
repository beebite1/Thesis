...
def plot_fundamental_diagrams(densities, flows, speeds, l, allData, measure):
    ...
    n = len(densities)
    threshold = 0
    densitiesS = [densities[i] for i in range(n) if 25 <= measure[i] <= 65]
    speedsS = [speeds[i] for i in range(n) if 25 <= measure[i] <= 65]
...
X = np.linspace(0+1e-3, 6, 100)
Y = -0.204*X + 1.48
Yw = 1.34*(1-np.exp(-1.913*(1/X - 1/5.4)))
Yf = slope*X + intercept

plt.plot(X, Y, label = "Speed-Density relation according to (Mōri and Tsukaguchi, 1987)", color = "black", linestyle = "--")
plt.plot(X, Yw, label = "Speed-Density relation according to (Weidmann, 1992)", color = "black", linestyle = "-.")
plt.plot(X, Yf, label = "Linear regression of the data")
plt.xlabel('Density $[m^{-2}]$')
plt.ylabel('Speed $[m/s]$')
#plt.ylabel('Flow [1/(m s)]')
#plt.axvline(x = 20, color = 'black', label = 'Stationary state interval')
