...
def plot_fundamental_diagrams(densities, flows, speeds, l, allData, measure):
    ...
    flowsS = [flows[i]/dists[l-1] for i in range(n) if 20 <= measure[i] <= 60]
    ...
    plt.scatter(densitiesS, flowsS, c = colors[l-1], label = label, marker=markers[l-1])
    #plt.scatter(measure, densities, c = colors[l-1], label = label, marker=markers[l-1])
    allData.append([densitiesS, flowsS])
...
#Y = -0.204*X + 1.48
Y = 1.34*(1-np.exp(-1.913*(1/X - 1/5.4)))
Yf = slope*X + intercept

plt.plot(X, Y, label = "Flow-Density relation according to (Weidmann, 1992)", color = "black")
#plt.plot(X, Yf, label = "Linear regression of the data")
plt.xlabel('Density [1/m^2]')
plt.ylabel('Flow [person/(m s)]')
