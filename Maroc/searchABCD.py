...
def getDensity(iterations):
    ...
    return np.array(densities)/1.28
...
def grid_search(iterations):
    ...
    r = 0.2*np.ones((num_pedestrians))*np.sqrt(1.28) # Pedestrian radius

    gridResol = 10
    A = np.round(np.linspace(0.15, 3.75, gridResol),2)
    A = [A[arg1]]
    B = np.round(np.linspace(0.15, 1.05, gridResol),2)
    B = [B[arg2]]
    ...
    D = [D[arg4]]
    D = [0.2]
    L = np.round(np.linspace(0.1, 1, gridResol),2)
    L = L[arg5]
