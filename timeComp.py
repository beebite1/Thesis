...
def getValues(scenario, id):
    ...
    colors = ['brown', 'purple', 'red', 'green', 'yellow']
    plt.hist(values, bins=int(np.sqrt(len(values)))+1,
            density=False, alpha=0.5, color=colors[id-1], edgecolor = "black",
            label="Scenario "+ str(id))

    # Add labels and title
    plt.xlabel('Time [s]')
    plt.ylabel('Count')
    plt.title('Histogram of Average Escape Time for each Scenario')
    #plt.grid(True)
    plt.legend()
    # Show plot
    #plt.show()
...
plt.errorbar(prctDirectives, meanTimes, stds, linestyle="None", marker = "D", markersize = 10, markerfacecolor = 'black', markeredgecolor = "black", capsize=15, ecolor='black', label = "Mean average times with standard deviation", linewidth = 3)
#plt.plot(prctDirectives, meanTimes,  linestyle = "--")
for i in range(len(prctDirectives)):
    plt.scatter(prctDirectives[i]*np.ones(len(allValls[i])), allValls[i], label = "Sample escape times in scenario "+str(ids[i]), color = colors[i])
