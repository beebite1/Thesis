        ...
        if filename.startswith(file):
            ...
            with open(file_path) as file:
                ...
                if len(lines) >= 2:
                    ...
                    try:
                        last_value = float(second_line[1])
...
stds = [np.std(val) for val in allValls]
...
plt.figure()
plt.title("Maximum acceptable arrival rate in each Scenario")
plt.scatter(prctDirectives, arrivals, linestyle="None", marker = "D", s= 100, label = "Average maximum acceptable arrival rate")
